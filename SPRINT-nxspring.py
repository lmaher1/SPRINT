...
layout = [
    ...
    [sg.T('Node Size:'), sg.Radio('Medium',"node_size", enable_events=True, default=True, key=("-size1-")),
     sg.Radio('Small',"node_size", enable_events=True, key=("-size2-")),
     sg.Radio('Large',"node_size", enable_events=True, key=("-size3-")),
     ],
...
#sets the variable n_size to differenct sizes depending on the users choice of node size
        
    if values["-size1-"]==True:
        n_size = 60
    elif values["-size2-"]==True:
        n_size = 30
    elif values["-size3-"]==True:
        n_size = 120
...
def SPLINTER(lst):
    ...
    # Generating a string for the GUI depending on the value of double_greater_error
    if double_greater_error == False:
        text_to_write = "This network is optimal provided the initialisation network is optimal."
    elif double_greater_error == True:
        text_to_write = "This network is not optimal since operation a) has been used twice in a row. To make optimal, replace any ladders as shown in Huber and Maher (2022) Fig. 10 with a bead. " 
        
        
        
    return [G,total_hybrid_number,text_to_write]
    
# read data from GUI data entry window as i[0] and i[1] then perform the operations to generate networkx graph
# and species list
G,tot_hybrids,text_to_write = SPLINTER([int(i[1]) for i in tuples])
species = [i[0] for i in tuples]

#Fig drawing material
weights = nx.get_edge_attributes(G,'weight').values()
#pos = graphviz_layout(G, prog='dot')
pos = nx.spring_layout(G, seed =828282)
...
nx.draw_networkx_nodes(G, pos, node_color = n_col, node_size = n_size)
...
layout = [
        ...
        [sg.Text(f"{text_to_write}")],
        [sg.B('Save Image'), sg.B('Save .dot File'), sg.Exit('Exit', size=(5, 1))]
         ]
...
while True:
    ...
    if event == sg.WIN_CLOSED or event == 'Exit':
        break   
    elif event == 'Save .dot File':
        sg.Popup("Unfortunately, DOT format is not available in SPRINT-nxspring.py as it relies on pygraphviz. SPRINT.py must be used to obtain one.")
    else:
